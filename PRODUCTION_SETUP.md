...
| **Каталог `/catalog`** | 🔴 **список зашитий у код** і не звертається до БД; його картки ведуть на `/catalog/1`…`/catalog/6`, яких у базі не існує → **404 на кожній картці** |
...
>
> ⚠️ **`/catalog` взагалі не показник стану БД** — список виконавців зашитий масивом
> у `src/app/catalog/page.tsx` (жодного звернення до Prisma). Сторінка виглядає однаково
> й при повністю мертвій базі. Справжня перевірка БД — реєстрація або сторінка виконавця
> `/catalog/<cuid>`, яка читає дані через `getDb()`.
